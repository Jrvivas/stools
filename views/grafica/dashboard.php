        ...
        function getReporte(codigo,...parameters){
            ...
            switch(codigo){
                case 'VENTA-MES':
                    ...
                    _titulo='Ventas del Mes'
                    query=`
                       SELECT pt.nombre, sum(dp.monto) as 'monto',sum(dp.alto*dp.ancho*dp.cantidad) as 'sup' 
                ...
                case 'GANANCIAXMES':
                    ...
                    if(parameters.length>0){
                     ...
                     query=`
                        SELECT pt.nombre, (sum(dp.monto) - sum(dp.alto*dp.ancho*dp.cantidad*pt.costo)) as monto  
                        FROM detallePedido as dp LEFT JOIN pedido as p 
                        ON p.id=dp.pedido_id AND p.app_idApp= dp.app_idApp 
                        LEFT JOIN  productos as pt 
                        ON dp.productos_id=pt.id AND pt.app_idApp= dp.app_idApp 
