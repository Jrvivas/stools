...
function listaProductos() {
    var a=new Server; 
    a.consulta('index.php?r=productos%2Flista-ajax&idApp='+idApp,{'_csrf': yii.getCsrfToken()},function(rst){
  ...
  function  listarProductosCateg(codigo){
    let cod=codigo.replace('cat_','');
    (new Server).consulta('index.php?r=productos%2Flista-ajax&idApp='+idApp+'&catCodigo='+cod,{'_csrf': yii.getCsrfToken()},function(rst){
    ...
    let htmltext=`<div class="row">
                        <div class="col-md-12 mt-5 ">
                            <div class="row marco_app">
                                <h2 id="titulo-lista" class="text-center">Productos</h2>
     ...
     if($("#pedido-contacto_id").val()!=''){
            ...
            $("#idModalPedido").show();
             ...
             $("#ModalPedidosLabel").html('CATEGORIAS'); 
             $("#titulo-lista").html('CATEGORIAS'); 
             //listaProductos(); 
             listaCategorias();
     }else{
         $("#idModalPedido").html("<h2>Por favor seleccione un cliente, gracias</h2>");  
...
function dibujarBotonesCateg(categorias){
      ...
      //--------boton para sin categoria----------
      $("#lista_Productos").append( "<button id=\"cat_"+idBot+"\" type=\"button\" class=\"btn btn-success \" style=\"margin: 5px;\"  >-SIN CATEGORIA-</button>");
      //-------------------    
      $("#cat_"+idBot).click(function(){
          //alert('Dibujar el dialogo para '+Cat.nombre+ ' id:'+Cat.id)
         listarProductosCateg(this.id)
      })
  if(categorias){
      for(Cat of categorias){
          idBot="cat_"+Cat.codigo;
          ...
          $("#lista_Productos").append( "<button id=\""+idBot+"\" type=\"button\" class=\"btn btn-success \" style=\"margin: 5px;\"  >"+Cat.nombre+"</button>");
    ...
    function dibujarBotones(productos){
          if(productos){
            $("#ModalPedidosLabel").html('PRODUCTOS'); 
            $("#titulo-lista").html('PRODUCTOS'); 
            $("#lista_Productos").html('');
            let index=1;
            idBot="";
    ...
    //===========================================================
 function formHtmlUnidad(){
        let body=''+
        '<p>'+productoSel.descripcion+'</p>'+
        '<h5 id="lbl_precio" >$'+productoSel.precio+'</h5>'+
        input('detalle-pedido-cantidad','cantidad','0',3)+
        input('detalle-pedido-unidad','unidades','1',3)+
 ...
 function formHtmlM2(){
    ...
    function calcularMostrarTotalPedido(){
        ...
        document.querySelector("#pedido-monto").value=total.toFixed(2);
    }
 ...
 */
    function addDetalle(){
        ...
        let desc=$("#detalle-pedido-descripcion").val();
    ...
    function dibujarDetallesPedido(){
       // detallesPedido
        document.querySelector("#detallesPedido table tbody").innerHTML="";
        let rows='';
        let i=0;
        for(r of detalles){
            if( r.estado==='ACTIVO'||r.estado==='NOEDIT'){
                let bot='     <td><button id="bot_item_'+i+'" onclick="borrarItem('+i+')" type="button" class="btn btn-danger">x</button></td>'
                if(r.estado==='NOEDIT'){
                    bot='<td></td>'
                }
        ...
        document.querySelector("#detallesPedido table tbody").innerHTML=rows;
    ...
    function botEnviar(obj,tipoApp){
            ...
            let newPedido={"id":getVal("pedido-id"),
                           ...
                           "fechafin":getVal("pedido-fechafin"),// este es el indicador de que fue cerrado el pedido
                           ...
                           "comentario":getVal("pedido-comentarios"),
                           ...
                           "prioridad":getVal("pedido-prioridad"),
                           "detalles":detalles};

            if(getVal("pedido-fechafin")==='' && getVal("pedido-estado")==='ENTREGADO' && !confirm('Se actualizará el stock de los productos listados, ¿continuamos con la operación?')){
                ...
                return false;
                    ...
                    server.consulta('index.php?r=pedido%2Fcreate-ajax&idApp='+idApp,{'_csrf': yii.getCsrfToken(),'data':newPedido},function(rst){
    ...
    function botNewEnviar(obj){
        obj.disabled = true; // ponemos el boton inactivo para que no envia varias veces
        //obtener datos y verificar
        let newPedido={"id":getVal("pedido-id"),
                       "fechaini":getVal("pedido-fechaini"),
                       "fechaentrega":getVal("pedido-fechaentrega"),
                       "fechafin":getVal("pedido-fechafin")!=''?getVal("pedido-fechafin"):null,// este es el indicador de que fue cerrado el pedido
                       "contacto_id":getVal("pedido-contacto_id"),
                       "nombre":'Pedido_'+getVal("pedido-fechaini")+'_'+$("#pedido-contacto_id").find('option:selected').text().replace(' ','_'),
                       "delivery":getVal("pedido-delivery"),
                       "comentario":getVal("pedido-comentarios"),
        ...
        if( newPedido.fechafin==null && newPedido.estado==='ENTREGADO' && !confirm('Se actualizará el stock de los productos listados y no se podrá cambiar de estado, ¿continuamos con la operación?')){

            return false;
        }
       

                console.log('se enviará '+newPedido);     
                ...
                server.consulta('index.php?r=pedido%2Fcreate-ajax&idApp='+idApp,{'_csrf': yii.getCsrfToken(),'data':newPedido},function(rst){
                        ...
                        if(rst){
                            ...
                            if(rst.error>0){
                                ...
                                window.location.href ="index.php?r=pedido/index-react&idApp="+idApp;
                            }
    ...
    function generarComprobante(){
        ...
        $("#idModalPedido").html(""+continer(body)+"")
            ...
            image.onload = function()
            {
                ...
                ctx.stroke();
                ctx.font = '14px serif';
                ctx.fillText('Recargo: '+getVal("pedido-impuesto"), mg, altoM+rgl*(30));
                ctx.fillText('Descuento: '+getVal("pedido-descuento"), mg, altoM+rgl*(31));
                ctx.fillText('Pagado: $'+getVal("pedido-pago"), mg, altoM+rgl*(33));
                ctx.font = '18px serif';
                ctx.fillText('Total: $'+getVal("pedido-monto"), mg, altoM+rgl*(35))
                ctx.font = '20px serif';
                ctx.fillText('saldo: $'+getVal("pedido-saldo"), mg+col*2, altoM+rgl*(35));
